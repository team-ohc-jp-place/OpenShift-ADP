...
ここでは、`i1.ocp46.example.localdomain`、`i2.ocp46.example.localdomain`、`i3.ocp46.example.localdomain`という3本の`Infraノード`を作成します。ここで作成する`Infraノード`のスペックは[「3-2-2. 周辺インフラ・サーバーのサイジング」](../3_Planning/README.md#3-2-2-周辺インフラ・サーバーのサイジング)で設計した通りに、以下の通りです。
...
```
dig +short i1.ocp46.example.localdomain   # ノードが無いと ping は返ってこないが、dig で名前解決はできる
dig +short i2.ocp46.example.localdomain   # ノードが無いと ping は返ってこないが、dig で名前解決はできる
dig +short i3.ocp46.example.localdomain   # ノードが無いと ping は返ってこないが、dig で名前解決はできる
...
```
# oc label node i1.ocp46.example.localdomain node-role.kubernetes.io/infra=
# oc label node i2.ocp46.example.localdomain node-role.kubernetes.io/infra=
# oc label node i3.ocp46.example.localdomain node-role.kubernetes.io/infra=
```
...
i1.ocp46.example.localdomain   Ready    infra,worker   15h   v1.19.0+3b01205
i2.ocp46.example.localdomain   Ready    infra,worker   15h   v1.19.0+3b01205
i3.ocp46.example.localdomain   Ready    infra,worker   15h   v1.19.0+3b01205
m1.ocp46.example.localdomain   Ready    master         15h   v1.19.0+3b01205
m2.ocp46.example.localdomain   Ready    master         15h   v1.19.0+3b01205
m3.ocp46.example.localdomain   Ready    master         15h   v1.19.0+3b01205
s1.ocp46.example.localdomain   Ready    infra,worker   66m   v1.19.0+3b01205
s2.ocp46.example.localdomain   Ready    infra,worker   66m   v1.19.0+3b01205
s3.ocp46.example.localdomain   Ready    infra,worker   66m   v1.19.0+3b01205
w1.ocp46.example.localdomain   Ready    worker         15h   v1.19.0+3b01205
w2.ocp46.example.localdomain   Ready    worker         15h   v1.19.0+3b01205
w3.ocp46.example.localdomain   Ready    worker         15h   v1.19.0+3b01205
...
backend machine_config
    mode tcp
    balance     roundrobin
    server bs bs.ocp46.example.localdomain:22623 check
    server m1 m1.ocp46.example.localdomain:22623 check
    server m2 m2.ocp46.example.localdomain:22623 check
    server m3 m3.ocp46.example.localdomain:22623 check

backend worker_http
    mode tcp
    balance     source
    server i1 i1.ocp46.example.localdomain:80  check    # 追加
    server i2 i2.ocp46.example.localdomain:80  check    # 追加
    server i3 i3.ocp46.example.localdomain:80  check    # 追加
    server w1 w1.ocp46.example.localdomain:80  check
    server w2 w2.ocp46.example.localdomain:80  check
    server w3 w3.ocp46.example.localdomain:80  check

backend worker_https
    mode tcp
    balance     source
    server i1 i1.ocp46.example.localdomain:443  check    # 追加
    server i2 i2.ocp46.example.localdomain:443  check    # 追加
    server i3 i3.ocp46.example.localdomain:443  check    # 追加
    server w1 w1.ocp46.example.localdomain:443  check
    server w2 w2.ocp46.example.localdomain:443  check
    server w3 w3.ocp46.example.localdomain:443  check
```
...
```
oc adm taint node i1.ocp46.example.localdomain infra=reserved:NoSchedule infra=reserved:NoExecute
oc adm taint node i2.ocp46.example.localdomain infra=reserved:NoSchedule infra=reserved:NoExecute
oc adm taint node i3.ocp46.example.localdomain infra=reserved:NoSchedule infra=reserved:NoExecute
```
...
```.yaml
<省略>
CreationTimestamp:  Sat, 2 Feb 2021 01:00:40 +0900
...
status:
  conditions:
  - lastTransitionTime: "2021-02-01T08:51:12Z"
    reason: AsExpected
    status: "False"
<省略>
```
...
image-registry   4.6.13    True        False         True       6m17s
```
...
Node:                 i1.ocp46.example.localdomain/172.16.0.41
...
Node:                 i3.ocp46.example.localdomain/172.16.0.43
[root@bastion openshift]# 
```
...
<a href="https://access.redhat.com/documentation/en-us/openshift_container_platform/4.6/html/monitoring/cluster-monitoring">参考: 1.1. About cluster monitoring</a>
...
backend infra_http                          # 名前の変更
    mode tcp
    balance     source
    server i1 i1.ocp46.example.localdomain:80  check    
    server i2 i2.ocp46.example.localdomain:80  check   
    server i3 i3.ocp46.example.localdomain:80  check  

backend infra_https                           # 名前の変更
    mode tcp
    balance     source
    server i1 i1.ocp46.example.localdomain:443  check    
    server i2 i2.ocp46.example.localdomain:443  check    
    server i3 i3.ocp46.example.localdomain:443  check    
```
